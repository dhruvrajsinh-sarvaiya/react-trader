...
class BuyOrderRow extends Component {
	...
	render() {
		...
		var lastClass = '';
		var change;
		change = parseFloat(this.props.OldLTP) - parseFloat(this.props.Price);
		
		if (this.props.Price !== this.props.OldLTP) {
			if(tempI < 50){
				//console.log("OldLTP:",this.props.OldLTP);
				//console.log("LTP:",this.props.LTP);
				//console.log("exchangeName:",this.props.exchangeName);
				tempI++;
			}
			//lastClass = change < 0 ? 'last-negative' : 'last-positive';
			//lastClass = change < 0 ? 'last-negative' : 'newRow';
			lastClass = 'newRow';
		}
		...
		return (
			...
			>
				...
				<td >{this.props.Price !== '-' ? parseFloat(this.props.Price).toFixed(this.props.priceLength) : '-'}</td>
