...
class SellTrade extends Component {
	...
	render() {		
		...
		var {sellerOrderList,exchangeList} = this.props;

		sellerOrderList.map((exchange,index) => {
			if(exchangeList[exchange.ProviderName.toLowerCase()] && exchangeList[exchange.ProviderName.toLowerCase()].LTP && exchangeList[exchange.ProviderName.toLowerCase()].OldLTP) {
				sellerOrderList[index].OldLTP = exchangeList[exchange.ProviderName.toLowerCase()].OldLTP;
				sellerOrderList[index].LTP = exchangeList[exchange.ProviderName.toLowerCase()].LTP;
			}						
		})

		sellerOrderList.sort(function (a, b) {
		...
		var countSell = (sellerOrderList.length - 1);
		var keyIndex = 0;
		const diffLimit = buySellRecordCount - sellerOrderList.length;

		sellerOrderList.map((newSellOrder, indexValue) => {
		...
		if (diffLimit <= buySellRecordCount) {
			for (var lastIndex = sellerOrderList.length; lastIndex < buySellRecordCount; lastIndex++) {
				keyIndex <= 8 ?
